...
class RealNVP(nn.Module):
    ...
    def inverse(self, y):
        ...
        # Undo swaps and coupling layers in reverse order
...
for epoch in range(epochs):
    ...
    # Discrete objective only defines the score weights
    with torch.no_grad():
        x = discretize(y)
        objective = f(x)
    ...
    # Hold y fixed and differentiate log p_theta(y)
    y_fixed = y.detach()
    ...
    print(
        f"Epoch {epoch:4d} | "
        f"mean f = {eval_mean_f:.6f} | "
        f"best f = {eval_best_f:.6f} | "
        f"P(opt) = {100 * p_optimal:5.2f}% | "
        f"f<0.01 = {100 * hit_001:5.2f}% | "
        f"mean = ({mean_point[0]:+.3f}, {mean_point[1]:+.3f}) | "
        f"lr = {current_lr:.2e}"
    )
...
# Validate inverse and density on the best model
with torch.no_grad():
    test_r = torch.randn(10000, 2)
...
print("P(optimal grid point):", 100 * p_optimal, "%")
...
plt.scatter(
    ...
    s=10,
    alpha=0.25,
    label="discrete RealNVP samples",
)
...
plt.title(
    "2D RealNVP with Gaussian base\n"
    f"best epoch = {best_epoch}, mean f = {plot_mean_f:.5f}"
)
